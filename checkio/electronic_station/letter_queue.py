...
"""
In computer science, a queue is a particular kind of data type in which the entities
in the collection are kept in order and the principal operations on the collection
are the addition of entities to the rear terminal position (enqueue or push),
...
which remain in the queue in one word from the front to the rear of the queue.
...
How it is used: Queues provide services in computer science, transportation,
and operations research where various entities such as data, objects, persons,
or events are stored and held to be processed later. In these contexts,
...
​
letter_queue=lambda z:reduce(lambda x,y:(x+y[5:])[not y[5:]:], z, '')
'''